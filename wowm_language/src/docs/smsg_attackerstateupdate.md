...
| - | 1 / - | u8 | amount_of_damages |  | Forced to be one in order to correctly handle struct depending on `hit_info`. |
| - | 4 / Little | u32 | spell_school_mask |  |  |
| - | 4 / Little | f32 | damage_float |  | arcemu sends the same data in `damage_uint`. |
| - | 4 / Little | u32 | damage_uint |  | arcemu sends the same data in `damage_float`. |
