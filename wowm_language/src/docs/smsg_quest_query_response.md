...
| - | 4 / Little | u32 | quest_level |  |
| - | 4 / Little | u32 | minimum_quest_level | min required level to obtain (added for 3.3).<br/>Assumed allowed (database) range is -1 to 255 (still using uint32, since negative value would not be of any known use for client) |
