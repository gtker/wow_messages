...
impl SMSG_TRADE_STATUS {
    fn read_inner(mut r: &mut &[u8], body_size: u32) -> Result<Self, crate::errors::ParseErrorKind> {
        if !(4..=13).contains(&body_size) {
            return Err(crate::errors::ParseErrorKind::InvalidSize { opcode: 0x0120, size: body_size });
        }
...
impl crate::Message for SMSG_TRADE_STATUS {
    ...
    fn to_test_case_string(&self) -> Option<String> {
        ...
        match &self.status {
            ...
            crate::wrath::SMSG_TRADE_STATUS_TradeStatus::CloseWindow {
                inventory_result,
                item_limit_category_id,
                target_error,
            } => {
            ...
            crate::wrath::SMSG_TRADE_STATUS_TradeStatus::OnlyConjured {
                slot,
            } => {
                writeln!(s, "    slot = {};", slot).unwrap();
            }
            crate::wrath::SMSG_TRADE_STATUS_TradeStatus::NotOnTaplist {
                slot,
            } => {
                writeln!(s, "    slot = {};", slot).unwrap();
            }
