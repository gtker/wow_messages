...
impl CMSG_MESSAGECHAT {
    pub fn to_test_case_string(&self) -> Option<String> {
        ...
        // Members
        writeln!(s, "    chat_type = {};", crate::wrath::ChatType::try_from(self.chat_type.as_int()as u8).unwrap().as_test_case_value()).unwrap();
        writeln!(s, "    language = {};", self.language.as_test_case_value()).unwrap();
        match &self.chat_type {
            crate::wrath::CMSG_MESSAGECHAT_ChatType::Whisper {
                target_player,
            } => {
                writeln!(s, "    target_player = \"{}\";", target_player).unwrap();
            }
            crate::wrath::CMSG_MESSAGECHAT_ChatType::Channel {
                channel,
            } => {
