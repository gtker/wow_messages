...
impl crate::Message for CMSG_MOVE_SET_COLLISION_HGT_ACK {
    ...
    fn to_test_case_string(&self) -> Option<String> {
        ...
        writeln!(s, "test CMSG_MOVE_SET_COLLISION_HGT_ACK {{").unwrap();
        // Members
        ...
        writeln!(s, "    }};").unwrap();
        writeln!(s, "    new_height = {};", if self.new_height.to_string().contains('.') { self.new_height.to_string() } else { format!("{}.0", self.new_height) }).unwrap();

        writeln!(s, "}} [").unwrap();

        let [a, b] = (u16::try_from(self.size() + 4).unwrap()).to_be_bytes();
        writeln!(s, "    {a:#04X}, {b:#04X}, /* size */").unwrap();
        let [a, b, c, d] = 1303_u32.to_le_bytes();
        ...
        crate::util::write_bytes(&mut s, &mut bytes, 4, "new_height", "    ");
    ...
    fn read_body<S: crate::private::Sealed>(r: &mut &[u8], body_size: u32) -> Result<Self, crate::errors::ParseError> {
        Self::read_inner(r, body_size).map_err(|a| crate::errors::ParseError::new(1303, "CMSG_MOVE_SET_COLLISION_HGT_ACK", body_size, a))
    }
