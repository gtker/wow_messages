...
use crate::Guid;
use crate::wrath::{
    MovementFlags, SplineFlag, TransportInfo, UpdateFlag, Vector3d, Vector4d,
};

/// Auto generated from the original `wowm` in file [`wow_message_parser/wowm/world/gameobject/smsg_update_object_3_3_5.wowm:72`](https://github.com/gtker/wow_messages/tree/main/wow_message_parser/wowm/world/gameobject/smsg_update_object_3_3_5.wowm#L72):
/// ```text
            ...
            match if_statement {
                MovementBlock_UpdateFlag_Living::Living {
                    ...
                    if let Some(if_statement) = &flags.spline_enabled {
                        ...
                        // duration_mod: f32
                        w.write_all(&if_statement.duration_mod.to_le_bytes())?;

                        // duration_mod_next: f32
                        w.write_all(&if_statement.duration_mod_next.to_le_bytes())?;
                        ...
                        // mode: u8
                        w.write_all(&if_statement.mode.to_le_bytes())?;
                ...
                MovementBlock_UpdateFlag_Living::Position {
                    ...
                    crate::util::vanilla_tbc_wrath_vector4d_write_into_vec(&transport_offset, &mut w)?;
                ...
                MovementBlock_UpdateFlag_Living::HasPosition {
                    ...
                    // position2: Vector4d
                    crate::util::vanilla_tbc_wrath_vector4d_write_into_vec(&position2, &mut w)?;
    ...
    pub(crate) fn read<R: std::io::Read>(mut r: R) -> Result<Self, crate::errors::ParseErrorKind> {
        ...
        let update_flag_living = if update_flag.is_living() {
            ...
            let flags_spline_enabled = if flags.is_spline_enabled() {
                ...
                // duration_mod: f32
                let duration_mod = crate::util::read_f32_le(&mut r)?;

                // duration_mod_next: f32
                let duration_mod_next = crate::util::read_f32_le(&mut r)?;

                // vertical_acceleration: f32
                let vertical_acceleration = crate::util::read_f32_le(&mut r)?;

                // effect_start_time: f32
                let effect_start_time = crate::util::read_f32_le(&mut r)?;
            ...
            // position1: Vector3d
            let position1 = crate::util::vanilla_tbc_wrath_vector3d_read(&mut r)?;

            // transport_offset: Vector4d
            let transport_offset = crate::util::vanilla_tbc_wrath_vector4d_read(&mut r)?;
            ...
            // position2: Vector4d
            let position2 = crate::util::vanilla_tbc_wrath_vector4d_read(&mut r)?;
...
impl MovementBlock_SplineFlag_FinalAngle {
    pub(crate) const fn as_int(&self) -> u32 {
        match self {
            Self::FinalAngle { .. } => 131072,
            Self::FinalTarget { .. } => 65536,
            Self::FinalPoint { .. } => 32768,
        }
    }
...
impl std::fmt::Display for MovementBlock_MovementFlags_OnTransportAndInterpolatedMovement {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
...
impl MovementBlock_MovementFlags_SplineEnabled {
    pub(crate) fn size(&self) -> usize {
        ...
        + 4 // duration: u32
        + 4 // duration_mod: f32
        + 4 // duration_mod_next: f32
        + 4 // effect_start_time: f32
    ...
    pub(crate) fn size(&self) -> usize {
        match self {
            ...
            Self::Position {
                ...
                + 16 // transport_offset: Vector4d
            }
            Self::HasPosition {
                ..
            } => {
                // Not an actual enum sent over the wire
                16 // position2: Vector4d
            }
        }
    }
}
