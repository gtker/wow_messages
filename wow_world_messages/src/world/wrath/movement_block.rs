...
impl MovementBlock {
    pub(crate) fn write_into_vec(&self, w: &mut Vec<u8>) -> Result<(), std::io::Error> {
        ...
        w.write_all(&(self.update_flag.as_int() as u16).to_le_bytes())?;
        ...
        if let Some(if_statement) = &self.update_flag.living {
            match if_statement {
                ...
                MovementBlock_UpdateFlag_Living::Position {
                    ...
                    transport_guid.write_packed_guid_into_vec(w);

                    // position1: Vector3d
                    position1.write_into_vec(w)?;
...
impl MovementBlock {
    pub(crate) fn read<R: std::io::Read>(r: &mut R) -> std::result::Result<Self, crate::errors::ParseError> {
        ...
        let update_flag_ROTATION = if update_flag.is_ROTATION() {
            // packed_local_rotation: u64
            let packed_local_rotation = crate::util::read_u64_le(r)?;
