...
impl AddonInfo {
    pub(crate) fn write_into_vec(&self, mut w: impl Write) -> Result<(), std::io::Error> {
        ...
        assert_ne!(self.addon_name.as_bytes().iter().next_back(), Some(&0_u8), "String `addon_name` must not be null-terminated.");
...
impl AddonInfo {
    pub(crate) fn read<R: std::io::Read>(mut r: R) -> Result<Self, crate::errors::ParseErrorKind> {
        ...
        // addon_has_signature: u8
        let addon_has_signature = crate::util::read_u8_le(&mut r)?;

        // addon_crc: u32
        let addon_crc = crate::util::read_u32_le(&mut r)?;

        // addon_extra_crc: u32
        let addon_extra_crc = crate::util::read_u32_le(&mut r)?;
