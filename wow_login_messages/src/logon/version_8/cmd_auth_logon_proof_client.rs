...
impl CMD_AUTH_LOGON_PROOF_Client {
    pub fn to_test_case_string(&self) -> Option<String> {
        ...
        writeln!(s, "];").unwrap();
        writeln!(s, "    security_flag = {};", crate::logon::version_8::SecurityFlag::new(self.security_flag.as_int()).as_test_case_value()).unwrap();
        ...
        if let Some(if_statement) = &self.security_flag.get_matrix_card() {
            write!(s, "    matrix_card_proof = [").unwrap();
        ...
        if let Some(if_statement) = &self.security_flag.get_authenticator() {
            writeln!(s, "    amount_of_tokens = {};", if_statement.tokens.len()).unwrap();
            write!(s, "    tokens = [").unwrap();
            for v in if_statement.tokens.as_slice() {
                write!(s, "{v:#04X}, ").unwrap();
            }
            writeln!(s, "];").unwrap();
        }
        ...
        if let Some(if_statement) = &self.security_flag.get_authenticator() {
            crate::util::write_bytes(&mut s, &mut bytes, 1, "amount_of_tokens", "    ");
            crate::util::write_bytes(&mut s, &mut bytes, if_statement.tokens.len(), "tokens", "    ");
        }
