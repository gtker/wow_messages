...
impl ClientMessage for CMD_SURVEY_RESULT {
    ...
    >> where
        ...
        Self: 'async_trait,
     {
        Box::pin(async move {Self::tokio_read_inner(r).await.map_err(|kind| crate::errors::ParseError::new(4, "CMD_SURVEY_RESULT", kind))})
    }
    ...
    >> where
        ...
        Self: 'async_trait,
     {
        Box::pin(async move {Self::astd_read_inner(r).await.map_err(|kind| crate::errors::ParseError::new(4, "CMD_SURVEY_RESULT", kind))})
    }
