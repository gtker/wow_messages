...
impl CMD_AUTH_LOGON_CHALLENGE_Server {
    pub fn to_test_case_string(&self) -> Option<String> {
        ...
        match &self.result {
            crate::logon::version_3::CMD_AUTH_LOGON_CHALLENGE_Server_LoginResult::Success {
                crc_salt,
                generator,
                large_safe_prime,
                salt,
                ...
                writeln!(s, "];").unwrap();
                writeln!(s, "    security_flag = {};", crate::logon::version_3::SecurityFlag::try_from(security_flag.as_int()).unwrap().as_test_case_value()).unwrap();
                match &security_flag {
                    crate::logon::version_3::CMD_AUTH_LOGON_CHALLENGE_Server_SecurityFlag::Pin {
                        pin_grid_seed,
                        pin_salt,
                    } => {
                        writeln!(s, "    pin_grid_seed = {};", pin_grid_seed).unwrap();
                        write!(s, "    pin_salt = [").unwrap();
                        for v in pin_salt.as_slice() {
                            write!(s, "{v:#04X}, ").unwrap();
                        }
                        writeln!(s, "];").unwrap();
                    }
            ...
            crate::logon::version_3::CMD_AUTH_LOGON_CHALLENGE_Server_LoginResult::Success {
                ...
                match &security_flag {
                    crate::logon::version_3::CMD_AUTH_LOGON_CHALLENGE_Server_SecurityFlag::Pin {
                        ...
                        crate::util::write_bytes(&mut s, &mut bytes, pin_salt.len(), "pin_salt", "    ");
                    }
                    _ => {}
                }
        ...
        writeln!(s, "] {{").unwrap();
        writeln!(s, "    login_versions = \"3 5 6 7\";").unwrap();
        writeln!(s, "}}\n").unwrap();
