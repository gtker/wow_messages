...
use crate::rust_printer::{DefinerType, get_new_flag_type_name};
...
fn print_size_for_new_flag(s: &mut Writer, rd: &RustDefiner) {
    s.variable_size(rd.ty_name(), "size", rd.size_is_const_fn(), |s| {
...
fn print_types_for_new_flag(s: &mut Writer, rd: &RustDefiner) {
    for enumerator in rd.complex_flag_enumerators() {
        ...
        s.variable_size(&new_type_name, "size", const_fn, |s| {
