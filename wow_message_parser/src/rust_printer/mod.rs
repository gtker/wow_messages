...
impl Writer {
    ...
    pub(crate) fn impl_world_server_or_client_message(
        ...
        if compressed {
            ...
            unencrypted(self, "");
            self.closing_curly_newline(); // fn write_unencrypted

            self.wln("#[cfg(all(feature = \"sync\", feature = \"encryption\"))]");
            self.wln(format!("fn write_encrypted_{ty}<W: std::io::Write>("));

            self.inc_indent();
            self.wln("&self,");
            self.wln("w: &mut W,");
            self.wln(format!("e: &mut {encrypter},"));
            self.dec_indent();
            ...
            self.open_curly(") -> Result<(), std::io::Error>");
            encrypted(self, "");
            self.closing_curly_newline(); // ) -> Result
            ...
            for async_ty in ImplType::types() {
                ...
                self.wln("w: &'w mut W,");
                self.dec_indent();
                self.wln(") -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), std::io::Error>> + Send + 'async_trait>>");
                self.wln("where");
                ...
                self.open_curly(""); // fn body

                self.open_curly("Box::pin(async move");
                unencrypted(self, ".await");
                self.closing_curly_with(")");

                self.closing_curly_newline(); // fn body

                self.wln(async_ty.cfg_and_encryption());
                self.wln(format!(
                ...
                self.wln("&'s self,");
                self.wln("w: &'w mut W,");
                self.wln(format!("e: &'e mut {encrypter},"));
                self.dec_indent();

                self.wln(format!(") -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), std::io::Error>> + Send + 'async_trait>>"));
                self.wln("where");
                ...
                self.open_curly("Box::pin(async move");
                encrypted(self, ".await");
                self.closing_curly_with(")");

                self.closing_curly_newline(); // fn body
            }

            self.closing_curly(); // impl {} for {}
            self.newline();
        } else {
            self.wln(format!(
                "impl {}::{} for {} {{}}",
