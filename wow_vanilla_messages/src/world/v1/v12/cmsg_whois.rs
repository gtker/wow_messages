...
impl MessageBody for CMSG_WHOIS {
    ...
    >> where
        ...
        Self: 'async_trait,
     {
        Box::pin(async move {
            // character: CString
            let character = crate::util::tokio_read_c_string_to_vec(r).await?;
            let character = String::from_utf8(character)?;
    ...
    >> where
        ...
        Self: 'async_trait,
     {
        Box::pin(async move {
            // character: CString
            w.write_all(self.character.as_bytes()).await?;
            // Null terminator
            w.write_all(&[0]).await?;
    ...
    >> where
        ...
        Self: 'async_trait,
     {
        Box::pin(async move {
            // character: CString
            let character = crate::util::astd_read_c_string_to_vec(r).await?;
            let character = String::from_utf8(character)?;
